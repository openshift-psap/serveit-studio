...
**Steps 4–5 — Capacity Analysis & Feasible Split Generation.** Calculates GPU cost per request from calibration data and derives ideal P/D split mathematically: `D_ideal = total_gpus / (r × prefill_tp + decode_tp)` where `r = decode_throughput / prefill_throughput`. Tests only ~3 candidates around the optimum per TP pair (floor/ceil/±1), reducing 132+ valid configurations on a 32-GPU cluster to ~6 tests — a 22× reduction. Filters asymmetric TP pairs where prefill_tp ≠ decode_tp by default to reduce search space (configurable via toggle — NIXL supports heterogeneous TP in both directions).
