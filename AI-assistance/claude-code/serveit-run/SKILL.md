...
Ask these questions **one at a time** — wait for the answer before asking the next one. Never combine multiple questions in a single message.

**First, ask:** "Do you already have a running ServeIt Studio instance, or do we need to deploy one?"

1. **I have a running instance** — ask for the URL, then skip to Step 3 (model selection)
2. **Deploy a new one** — continue to the next question

**If deploying, ask:** "Quick setup or step-by-step?"

1. **Quick setup** — auto-detect everything, deploy and configure. User only picks the model.
2. **Step-by-step** — ask for each choice (namespace, storage class, GPU limits, instance name, etc.)

**Then ask:** "Which deployment mode?"
...
If you don't already have the repo cloned locally, clone it first:

```bash
git clone https://github.com/openshift-psap/serveit-studio.git
cd serveit-studio
```

All `deployment/deploy.py` commands below assume you are in the repo root.

### Local mode (single instance)

Requires: `kubectl`/`oc` configured with cluster access.

1. **Choose a storage class for the instance PVC.** This PVC stores the wizard UI's SQLite database, collected metrics, optimization logs, and test results. It does NOT store model weights — that is configured separately later in the optimization settings (Step 3f, Storage section), where the user also picks the model cache SC and optionally enables disk KV cache offloading. Make this distinction clear to the user when asking.

**Investigate ALL storage classes** before presenting them. Do NOT just list SC names — actually check what each one is backed by so you can give the user accurate descriptions.

```bash
# Get full SC details (provisioner, parameters, backing)
...
for sc in data['items']:
    ...
    pool = params.get('storagePool', '')
    device_class = params.get('topolvm.io/device-class', '')
    print(f'{name}: provisioner={prov}' + (f' pool={pool}' if pool else '') + (f' device-class={device_class}' if device_class else ''))
"

# Check PVs to understand what backs each SC (especially local disk SCs)
kubectl get pv --no-headers
```

Analyze the results to understand:
- **Which SCs share the same physical backing** — if one SC's `storagePool` parameter references another SC's name (e.g., `hostpath-nvme` has `storagePool: local-nvme`), they use the same physical disks. In that case, **present them as a single option** and explain the relationship (e.g., "hostpath-nvme (dynamically provisions from local NVMe disks)"). Do NOT show both as separate options — that confuses the user.
- Which SCs are dynamically provisioned vs static, and what size PVs they create
- **Whether each SC's provisioner is actually running and functional** — check that the provisioner pods exist and are healthy. For example, an NFS SC is useless if the NFS server pod isn't running. A CSI driver SC won't work if the CSI pods are missing.
- **Local disk support uses HostPath Provisioner (HPP) only** — ServeIt Studio's local disk feature (`local_disk_path`) requires an HPP-backed SC (provisioner `kubevirt.io.hostpath-provisioner`). Static local PV SCs (`kubernetes.io/no-provisioner`) are consumed by HPP as its backing pool — they should NOT be presented as a separate option. When you see a static local SC whose PVs are bound by HPP, merge it with the HPP SC into one option.

```bash
# Verify provisioners are running (check for provisioner pods across all namespaces)
kubectl get pods -A --no-headers | grep -i 'provisioner\|csi\|nfs\|topolvm'
```

Mark any SC whose provisioner is not running as "⚠️ provisioner not running" when presenting to the user. Still show it in the list — do NOT hide it — but warn clearly so the user can make an informed choice.

If the user picks an SC that has a broken provisioner and the PVC gets stuck Pending, explain what happened, clean up, and re-present ALL remaining SCs (not just a subset — always show the full list).
...
2. **Create admin account** (default: `admin / serveit`):

```bash
curl -sk -c /tmp/serveit-cookies.txt "$URL/setup" \
...
3. **Scan the cluster** — this is mandatory for BOTH local and launcher modes. The scan detects GPUs, networking, storage, and installed infrastructure. Show the results to the user:
...
# Scan
curl -sk -b /tmp/serveit-cookies.txt -X POST "$URL/api/scan" | python3 -c "
import json, sys
# IMPORTANT: The /api/scan response returns fields at the TOP LEVEL — not nested
# under 'resources' or 'cluster_resources'. Do NOT wrap with data.get('resources', ...).
data = json.load(sys.stdin)
print(f'GPU Model:      {data.get(\"gpu_model\", \"unknown\")}')
print(f'Total GPUs:     {data.get(\"total_gpus\", 0)}')
print(f'GPU Nodes:      {data.get(\"gpu_node_count\", 0)}')
print(f'GPU VRAM:       {round(data.get(\"gpu_memory_per_gpu_mb\", 0) / 1024, 1)} GB per GPU')
print(f'Max GPUs/node:  {data.get(\"max_gpus_per_node\", 0)}')
print(f'TP options:     {data.get(\"tp_options\", [])}')
print(f'RDMA:           {\"yes\" if data.get(\"has_rdma\") else \"no\"}')
print()
print('GPU Nodes:')
for n in data.get('nodes_detail', []):
    if n.get('gpus', 0) > 0:
        nics = ', '.join(f'{nic[\"count\"]}x {nic[\"type\"]}' for nic in n.get('nics', []))
        print(f'  {n[\"name\"]}: {n[\"gpus\"]}x {n.get(\"gpu_model\",\"?\")} ({nics})')
print()
print('Available Networks:')
for net in data.get('available_networks', []):
    status = '✅' if net.get('available') else '❌'
    print(f'  {status} {net[\"name\"]} ({net[\"id\"]})')
"
```

Present the scan results to the user clearly — GPUs, nodes, RDMA capability, available networks, and any warnings.

4. **Ask the user to choose the network type** — present ONLY the networks that the scan found as available. Explain what each one does and which is best for their setup:

```
Which network type for pod communication?
1. DRA — GPU+NIC PCIe affinity via device classes (best RDMA performance)
2. NAD — Multus secondary networks
3. TCP — standard pod networking (no RDMA)
... (only show what's available)
```

If DRA is available with a `gpu_nic_pair` device class, recommend it. If only TCP is available, note that multi-node inference performance will be limited.

5. **Ask the user to choose model cache storage** — this is separate from the instance PVC. The model cache stores the actual model weights (can be 10-140GB+). Present all SCs again (same investigation as step 1) and explain the trade-offs for model storage specifically:
   - Local NVMe (hostpath-nvme): fastest model loading (~2-3 GB/s), per-node copy
   - NFS/shared: one copy shared across nodes, slower loading (20-40 min for large models)
   - Block storage: RWO only, won't work for multi-node

6. Continue to Step 3 (model selection).

### Launcher mode (multi-tenant)

Requires: `kubectl`/`oc` configured with cluster access.

1. **Investigate ALL storage classes** before presenting them — same as local mode: get full SC details (`kubectl get sc -o json`), check PVs (`kubectl get pv`), understand which SCs share the same physical backing, then present ALL as numbered options with accurate descriptions. Do NOT guess from provisioner names alone.
   - For the **launcher PVC** (small DB): any fast RWO SC works (LVMS, block storage, local disk).
   - For the **instance PVC**: stores the wizard UI's SQLite database, collected metrics, optimization logs, and test results. All data persists across pod restarts. Any fast RWO SC works. **Must pass `storage_class` explicitly** when creating the instance, otherwise it falls back to the cluster default SC which may not schedule correctly. When presenting this choice to the user, explain what the PVC is for: "This volume stores the wizard UI's database, collected metrics, optimization logs, and test results."
...
Ask: **"What optimization mode do you want to run?"**

**Full optimization modes** — these sweep TP values, pod counts, and architectures to find the best config:

1. **Response time** (`ttft`) — optimize for the shortest time to first token. Best for: chat, IDE, real-time apps where users wait for a response.
2. **Throughput** (`throughput`) — optimize for maximum requests per second. Best for: batch processing, CI pipelines, high-volume APIs.
3. **Full coverage** (`balanced`) — test for both latency and throughput across all architectures (Aggregated, PD, EP). Most tests, most complete picture. Best when you're not sure what matters most.

**Architecture-specific modes** — restrict the search to one architecture only:

4. **Aggregated only** (`aggregated_only`) — only test standard aggregated configs (all pods do both prefill and decode). Skips PD and EP.
5. **PD only** (`pd_only`) — only test prefill/decode disaggregated configs. Skips aggregated and EP.
6. **EP only** (`ep_only`) — only test expert parallel configs. Only for MoE models (Qwen3 MoE, DeepSeek V3/V4, Gemma 4, etc.).

**Single test mode** — run one exact configuration you specify, no sweeps:

7. **Single test** (`single_test`) — you specify the exact architecture, TP, and pod counts. Useful for re-testing a known config, validating a specific setup, or benchmarking a config from a previous run.

If the user picks **Single test (7)**, ask these follow-up questions one at a time:

**"Which architecture?"**
   1. Aggregated — all pods do everything
   2. PD — separate prefill and decode pods
   3. EP — expert parallel (MoE models only)

**For Aggregated or EP:**
   - "How many GPUs per pod (TP)?" — e.g., 1, 2, 4, 8
   - "How many replicas?" — e.g., 1, 2, 4

**For PD:**
   - "Prefill TP?" — GPUs per prefill pod (e.g., 4)
   - "Decode TP?" — GPUs per decode pod (e.g., 8)
   - "Prefill pods?" — number of prefill pods (e.g., 1, 2, 3)
   - "Decode pods?" — number of decode pods (e.g., 1)

   Example: PTP4 DTP8 with 3 prefill pods and 1 decode pod = `single_test_prefill_tp: 4, single_test_decode_tp: 8, single_test_prefill_pods: 3, single_test_decode_pods: 1`

Set in config:
- `goal: "single_test"`
- `single_test_architecture: "aggregated" | "pd" | "ep"`
- `single_test_tp: N` (for aggregated/EP)
- `single_test_replicas: N` (for aggregated/EP)
- `single_test_prefill_tp: N` (for PD)
- `single_test_decode_tp: N` (for PD)
- `single_test_prefill_pods: N` (for PD)
- `single_test_decode_pods: N` (for PD)

8. **Other** — describe your priority.
...
- **Yes, most requests share a system prompt** → Explain the options with recommended percentages:
  ...
  - **Shared prefix mode** — All requests share the first N% of tokens (like a system prompt), but the rest is unique. The percentage is the fraction of the prompt that's shared across all requests.
    - Coding assistant with shared repo/project context: **50-70%** (project context is ~half the total input)
...
Show the defaults first, then present the FULL options menu exactly as written below. Do NOT abbreviate, summarize, or compress this list — always show every single option with its number. This is the main settings menu that the user will reference throughout configuration.

```
Defaults:
- Test duration: 300s (5 min per test)
- Search depth: top 2 configs per architecture
- P/D search: smart (auto-calculated ratios)
- Latency SLA: none
- Calibrated load + concurrency sweep: yes (top 4 configs at 8 load levels)
- Cache sweep: no
- vLLM auto-tune: on
- EPP preset: cache_optimized (when prefix cache enabled), balanced (otherwise)
- Container images: latest GA

1. Use all defaults — start the optimization
2. Test duration — change how long each test runs
3. Search depth — how many GPU configurations to test
4. Latency SLA — set a maximum acceptable response time
5. Production load analysis — calibrated load + concurrency sweep settings
6. Cache hit sweep — test different cache hit ratios
7. vLLM auto-tune — engine settings
8. EPP routing — request routing strategy
9. Container images — vLLM and EPP versions
10. Network — pod communication, RDMA options
11. Storage — where model weights are stored
12. Other
```

If the user picks a number, jump to that specific section. After handling that section, show the FULL menu again so they can pick another option or choose 1 to start. If they pick 1, use defaults:
...
**Goal-specific fields** — add these to the config JSON above depending on the `goal` value:

For `goal: "aggregated_only"`, `"pd_only"`, or `"ep_only"` — no extra fields needed, just set the goal.

For `goal: "single_test"` — add the single test fields:

```json
// Single test: Aggregated TP4 with 2 replicas (8 GPUs)
{"goal": "single_test", "single_test_architecture": "aggregated", "single_test_tp": 4, "single_test_replicas": 2}

// Single test: PD with 3 prefill pods × TP4 + 1 decode pod × TP8 (20 GPUs)
{"goal": "single_test", "single_test_architecture": "pd", "single_test_prefill_tp": 4, "single_test_decode_tp": 8, "single_test_prefill_pods": 3, "single_test_decode_pods": 1}

// Single test: EP TP2 with 4 replicas (8 GPUs, MoE only)
{"goal": "single_test", "single_test_architecture": "ep", "single_test_tp": 2, "single_test_replicas": 4}
```

**Quick reference — complete config examples by goal:**

```json
// Balanced (default) — full sweep across all architectures
{"goal": "balanced", "model": "RedHatAI/Qwen2.5-Coder-32B-Instruct-FP8-dynamic", "isl": 2000, "osl": 2000, "users": 50, "max_gpus": 8, "tp_pair_top_n": 2, "calibrated_load_enabled": true, "inferencex_sweep_enabled": true}

// TTFT — optimize for latency only
{"goal": "ttft", "model": "meta-llama/Llama-3.1-8B-Instruct", "isl": 4000, "osl": 500, "users": 20, "max_gpus": 8}

// Throughput — optimize for req/s
{"goal": "throughput", "model": "RedHatAI/Meta-Llama-3.1-70B-Instruct-FP8-dynamic", "isl": 2000, "osl": 2000, "users": 100, "max_gpus": 16}

// Aggregated only — skip PD/EP
{"goal": "aggregated_only", "model": "Qwen/Qwen3-32B", "isl": 2000, "osl": 2000, "users": 50, "max_gpus": 8}

// PD only — skip aggregated/EP
{"goal": "pd_only", "model": "RedHatAI/Meta-Llama-3.1-70B-Instruct-FP8-dynamic", "isl": 2000, "osl": 2000, "users": 100, "max_gpus": 16}

// EP only — MoE models only
{"goal": "ep_only", "model": "RedHatAI/Qwen3-Coder-480B-A35B-Instruct-FP8", "isl": 2000, "osl": 2000, "users": 50, "max_gpus": 16}
...
After verification passes, show the confirmation summary (same format as the summary block in the skill — model, workload, settings, infrastructure) and include the wizard URL. Then remind the user:

**"Everything is set. You can access the wizard UI at:**
- **Wizard URL**: <INSTANCE_URL>

(In launcher mode, also show: **Launcher URL**: <LAUNCHER_URL>)
...
**You can download an HTML report at any time** — as soon as the very first test completes (even just TP1 in the calibration sweep), the report is available with whatever results have been collected so far. As more tests complete, the report grows with more data, charts, and recommendations. In the wizard UI, click the 💾 **Download** button in the sidebar to get a self-contained HTML file you can open offline or share with your team.
...
```bash
curl -sk -b /tmp/serveit-cookies.txt "$URL/api/runs/$RUN_ID/report" -o report.html
```

Download deployment manifests for a specific config:
