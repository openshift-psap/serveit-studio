...
# InfeRecipe Optimizer YAML Generator
...
show_help() {
    cat << 'EOF'
InfeRecipe Optimizer Deployment Script

Usage: ./deployment/deploy.sh [options]

By default, this script deploys InfeRecipe optimizer to your cluster automatically.
...
generate_yaml() {
cat << EOF
# Generated by InfeRecipe deployment/deploy.sh
# Namespace: ${NAMESPACE}
# Image: ${IMAGE}
# PVC: ${ACTUAL_PVC_NAME}
      ...
      containers:
        - name: runner
          ...
          command:
            ...
            - "-c"
            - |
              echo "--- Starting InfeRecipe Optimizer ---"
...
if [[ "$AUTO_DEPLOY" == "false" ]]; then
    ...
    echo "🚀 Deploying InfeRecipe Optimizer to namespace: ${NAMESPACE}" >&2
    echo "" >&2
    ...
    if ! $KUBECTL_CMD api-resources 2>/dev/null | grep -q leaderworkerset; then
        ...
        echo "   InfeRecipe requires LeaderWorkerSet for all pod deployments." >&2
        echo "" >&2
    ...
    if ! $KUBECTL_CMD get namespace istio-system &>/dev/null; then
        ...
        echo "   InfeRecipe requires Istio for Gateway API inference routing." >&2
        echo "" >&2
        echo "   Install Istio before deploying InfeRecipe." >&2
        echo "" >&2
        exit 1
    fi
...
json.dump(s, sys.stdout)" \
    ...
    if generate_yaml | $KUBECTL_CMD apply -f -; then
        ...
        echo "" >&2
        echo "============================================================" >&2
        echo "✅ InfeRecipe Optimizer deployment complete!" >&2
        echo "============================================================" >&2
        echo "" >&2
