...
# InferRecipe Optimizer YAML Generator
# Generates Kubernetes manifests for InferRecipe optimizer deployment
...
show_help() {
    cat << 'EOF'
InferRecipe Optimizer Deployment Script

Usage: ./deployment/deploy.sh [options]

By default, this script deploys InferRecipe optimizer to your cluster automatically.
...
generate_yaml() {
cat << EOF
# Generated by InferRecipe deployment/deploy.sh
# Namespace: ${NAMESPACE}
# Image: ${IMAGE}
# PVC: ${ACTUAL_PVC_NAME}
      ...
      containers:
        - name: runner
          ...
          command:
            ...
            - "-c"
            - |
              echo "--- Starting InferRecipe Optimizer ---"
...
if [[ "$AUTO_DEPLOY" == "false" ]]; then
    ...
    echo "🚀 Deploying InferRecipe Optimizer to namespace: ${NAMESPACE}" >&2
    echo "" >&2
    ...
    if ! $KUBECTL_CMD api-resources 2>/dev/null | grep -q leaderworkerset; then
        ...
        echo "   InferRecipe requires LeaderWorkerSet for all pod deployments." >&2
        echo "" >&2
    ...
    if ! $KUBECTL_CMD get namespace istio-system &>/dev/null; then
        ...
        echo "   InferRecipe requires Istio for Gateway API inference routing." >&2
        echo "" >&2
        echo "   Install Istio before deploying InferRecipe." >&2
        echo "" >&2
        exit 1
    fi
...
json.dump(s, sys.stdout)" \
    ...
    if generate_yaml | $KUBECTL_CMD apply -f -; then
        ...
        echo "" >&2
        echo "============================================================" >&2
        echo "✅ InferRecipe Optimizer deployment complete!" >&2
        echo "============================================================" >&2
        echo "" >&2
