...
def handle_stop_optimization():
    ...
    # 2. Kill remote guidellm + clean up test deployments
    ...
    try:
        ...
        pass
    # Delete test LWS deployments so greenlet can't redeploy
    try:
        subprocess.run(
            ['kubectl', 'delete', 'lws', '--all', '-n', ns, '--ignore-not-found'],
            capture_output=True, timeout=15, check=False)
    except Exception:
        pass

    # 3. Kill local kubectl subprocesses (except port-forward)
    def _kill_kubectl():
        try:
            subprocess.run(['bash', '-c', "ps aux | grep kubectl | grep -v port-forward | grep -v grep | awk '{print $2}' | xargs -r kill -9"],
                           capture_output=True, timeout=5, check=False)
...
def handle_recreate_storage(data):
    """Recreate PVCs and download model for a resumed run."""
    import json as _json
    run_id = data.get('run_id')
    hf_token = data.get('hf_token')

    if not run_id:
        emit('recreate_storage_done', {'run_id': run_id, 'error': 'No run_id provided'})
        return

    try:
        with get_db() as conn:
            row = conn.execute('SELECT config_json, model FROM optimization_runs WHERE id = ?', (run_id,)).fetchone()
        if not row or not row[0]:
            emit('recreate_storage_done', {'run_id': run_id, 'error': 'Run not found or no config'})
            return

        cfg = _json.loads(row[0])
        model_name = cfg.get('model_name') or row[1]
        namespace = cfg.get('namespace') or TARGET_NAMESPACE
        ...
        if not storage_class and not data.get('storage_class'):
            # Ask user to provide storage class
            import subprocess
            r = subprocess.run(['kubectl', 'get', 'sc', '-o', 'jsonpath={.items[*].metadata.name}'],
                              capture_output=True, text=True, timeout=10, check=False)
            available = r.stdout.strip().split() if r.returncode == 0 else []
            emit('recreate_storage_done', {
                'run_id': run_id,
        ...
        # Try config_json first (new runs store it unredacted)
        cfg_token = cfg.get('hf_token')
        if cfg_token and cfg_token != '***':
            hf_token = cfg_token
        # Fallback to ui_session_state
        if not hf_token or hf_token == '***':
            try:
                with get_db() as conn:
                    ss = conn.execute('SELECT config_json FROM ui_session_state WHERE id = 1').fetchone()
                    if ss and ss[0]:
                        ui_cfg = _json.loads(ss[0])
                        hf_token = ui_cfg.get('hf_token') or hf_token
            except Exception:
                pass

        if not hf_token or hf_token == '***':
            emit('recreate_storage_done', {'run_id': run_id, 'error': 'HuggingFace token not available. Set it in the config page first.'})
            return
        ...
        if per_node_storage and node_nfs_pvcs:
            ...
            job_yaml = tm.render_template('prereq/model-download-job.yaml.j2',
                job_name=job_name, namespace=namespace, test_id=test_id,
                model_name=model_name, pvc_name=pvc_name, hf_token=hf_token)
            subprocess.run([kubectl_cmd, 'delete', 'job', job_name, '-n', namespace, '--ignore-not-found'],
                          capture_output=True, text=True, timeout=15, check=False)
            subprocess.run([kubectl_cmd, 'apply', '-f', '-', '-n', namespace],
                          input=job_yaml, capture_output=True, text=True, timeout=30, check=False)
        ...
        log_to_ui(f"❌ Failed to recreate storage: {e}", 'error')
        emit('recreate_storage_done', {'run_id': run_id, 'error': str(e)})
