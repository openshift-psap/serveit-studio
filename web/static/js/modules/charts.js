...
function renderCharts(data, runId) {
    ...
    if (aggResults.length > 1 && document.getElementById(cid('chart-agg-ttft-all'))) {
        ...
        Plotly.newPlot(cid('chart-agg-ttft-all'), traces, {
            ...
            barmode: 'group',
            margin: { t: 30, b: 80, l: 60, r: hasAggItl ? 120 : 60 },
            xaxis: { title: 'Aggregated Configuration', domain: hasAggItl ? [0, 0.85] : [0, 1] },
            yaxis: { title: 'TTFT (ms) — lower is better', side: 'left', tickformat: '.2s' },
            yaxis2: { title: 'Throughput Mean (req/s)', side: 'right', overlaying: 'y', anchor: hasAggItl ? 'free' : 'x', position: hasAggItl ? 0.87 : 1, titlefont: { color: '#f59e0b' }, tickfont: { color: '#f59e0b' } },
