...
function renderCharts(data, runId) {
    ...
    if (data.epp_tuning && data.epp_tuning.by_architecture) {
        ...
        Object.keys(eppData.by_architecture).forEach((arch, archIdx) => {
            ...
            eppHtml += `<div style="background:#f8fafc; border-radius:10px; padding:16px; text-align:center; border:1px solid #e2e8f0;"><div style="font-size:1.2em; font-weight:800; color:#1e293b;">${bestTrial.weights || 'N/A'}</div><div style="color:#64748b; font-size:0.82em; margin-top:4px;">EPP Weights (C:K:Q:A)</div></div>`;
            eppHtml += '</div>';
