...
function renderCharts(data, runId) {
    ...
    if (aggResults.length > 1 && document.getElementById(cid('chart-agg-ttft-all'))) {
        ...
        Plotly.newPlot(cid('chart-agg-ttft-all'), traces, {
            ...
            xaxis: { title: 'Aggregated Configuration', domain: hasAggItl ? [0, 0.92] : [0, 1] },
            yaxis: { title: 'TTFT (ms) — lower is better', side: 'left', tickformat: '.2s' },
            yaxis2: { title: 'Throughput Mean (req/s)', side: 'right', overlaying: 'y', anchor: hasAggItl ? 'free' : 'x', position: hasAggItl ? 0.93 : 1, titlefont: { color: '#f59e0b' }, tickfont: { color: '#f59e0b' } },
